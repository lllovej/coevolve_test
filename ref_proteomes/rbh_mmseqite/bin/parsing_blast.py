...
def main():
    ...
    #file='test.m8'
    #absolute path    
    abs_blast='/home/j/juliezhu/pfs/coevolve_1st/ref_proteomes/rbh_mmseqite/after_parse/'
		
    df = pd.read_csv(abs_blast+file, delimiter='\t', header=None)
    df.columns = ['query','target','pident','alnlen','qstart','qend','tstart','tend','qcov','tcov','evalue']
    ...
    if len(df[df.duplicated(subset='target', keep=False)]) != 0:
        df,manual_df = multiTarget_trimming(df, manual_df)

    if len(df[df.duplicated(subset='query', keep=False)]) != 0:
        df, manual_df = multiQuery_trimming(df, manual_df)

    #save new df as the final csv.

    # df.to_csv(proteome+'.m8')
    df.to_csv(abs_blast+file,header=False,sep='\t',index=False)

    #check if manual_df is empty or not
    if not manual_df.empty:
       manual_df.to_csv(abs_blast+'manual/'+file,header=False,sep='\t',index=False)
...
def multiTarget_trimming(df,manual_df):
    ...
    df_dropt = df.groupby('target', sort=False).filter(lambda x: len(x) >= 3)
    df = df.drop(index=df_dropt.index)

    #get target proteins with two ecoli hits and groupby 'Tid'.
    g1 = df[df.duplicated(subset='target', keep=False)].sort_values('evalue').groupby('target', sort=False)
    ...
    for i in g1_name:
        ...
        sta1, end1 = g1_df.iloc[0]['tstart'], g1_df.iloc[0]['tend']
        sta2, end2 = g1_df.iloc[1]['tstart'], g1_df.iloc[1]['tend']
...
def multiQuery_trimming(df, manual_df):
    ...
    df_dropq = df.groupby('query', sort=False).filter(lambda x: len(x) >= 3)
    df = df.drop(index=df_dropq.index)

    # get ecoli proteins with two target hits and groupby 'Qid'.
    g2 = df[df.duplicated(subset='query', keep=False)].sort_values('evalue').groupby('query', sort=False)
