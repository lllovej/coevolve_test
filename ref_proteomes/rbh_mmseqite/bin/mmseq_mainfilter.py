...
def main():
    ...
    df.columns = ['qseqid', 'sseqid', 'pident','length', 'qstart', 'qend', 'tstart', 'tend','qcov', 'tcov', 'evalue']
    ...
    df = df[(df.length >= 50) | (df.qcov >= 0.5 )]
    df = df[(df.qcov >= 0.5) | (df.tcov >= 0.5)]
    
#    #apply for parallel filtering process
    if direction == 'forward':
#    #forward:
        abs_path = '/home/j/juliezhu/pfs/coevolve_1st/ref_proteomes/rbh_mmseqite/forward_trimmed/'	
        forward_hit = filtering(df)
        forward_hit.to_csv(abs_path+file, header=False, sep='\t', index=False)

    elif direction == 'backward':
        abs_path = '/home/j/juliezhu/pfs/coevolve_1st/ref_proteomes/rbh_mmseqite/backward_trimmed/'
        backward_hit = filtering(df)
        backward_hit.to_csv(abs_path+file,header=False,sep='\t',index=False)
...
def rank_identity(df1):
    ...
    for i in range(1, len(df1_rank)):
        ...
        judgement1 = (len(overlap) >= 0.25 * df1_rank.iloc[i]['length']) & (
                    df1_rank.iloc[i]['pident'] > df1_rank.iloc[0]['pident'] - 0.1)
        judgement2 = (len(overlap) < 0.25 * df1_rank.iloc[i]['length']) & (
                    df1_rank.iloc[i]['pident'] > df1_rank.iloc[0]['pident'] - 0.2)
