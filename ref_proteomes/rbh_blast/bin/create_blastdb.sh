...
proteome_list=/home/j/juliezhu/pfs/data/ref_proteomes/rbh/createdb_name
idx=$(expr $offset + $SLURM_ARRAY_TASK_ID)
f_name=$(sed -n ${idx}p $proteome_list)
db_name=$(echo $f_name| cut -d '.' -f 1)

cd alldata_db/
#crate blast database
makeblastdb -in ../../all_data/unzipped/$f_name -dbtype 'prot' -out $db_name
